...
tjis="u246cb4d9c8a981d4299b7ef16cad49d9"
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By : ☄i̺͆y̺͆o̺͆😎\nContact Me : 👉 http://line.me/ti/pVxtJPseezk",    
    "commentOn":True,
    "commentBlack":{},
    ...
    try:
                ...
                try:
                    if cctv['cyduk'][op.param1]==True:
                        if op.param1 in cctv['point']:
                          ...
                          #  Image = iyo.getContact(op.param2).pictureStatus
                            #image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus  
                            if Name in cctv['sidermem'][op.param1]:
                               # Image in cctv['sidermem'][op.param1]:
                                pass
                            else:
                              ...
                              #  cctv['sidermem'][op.param1] += "\n• " + Image
                                if " " in Name:
                                    ...
                                    if len(nick) == 2:
                                       ...
                                       # iyo.sendImageWithURL(msg.to,"http://dl.profile.line.naver.jp/"+ Image.pictureStatus)		    
                                    else:
                                        iyo.sendText(op.param1, "Haii " + "☞ " + nick[1] + " ☜" + "\nBetah Banget Jadi Penonton. . .\nChat Napa (-__-)   ")
                                        #iyo.sendImageWithURL(msg.to,"http://dl.profile.line.naver.jp/"+ nick.pictureStatus)		    
                                else:
        ...
        if op.type == 17:
          if wait["Sambutan"] == True:
            ...
            image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
            iyo.sendText(op.param1,"welkam ya " + iyo.getContact(op.param2).displayName + "\nwekam To -> " + str(ginfo.name) + " <-" + "\nlangsung cek ja membr nya... \nkali ja lu dapet anu...")
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in Creator:
                return
            iyo.sendText(op.param1,"selamat tinggal " + iyo.getContact(op.param2).displayName +  "\nSemoga tenang di alam sana yah...")
            d = Message(to=op.param1, from_=None, text=None, contentType=7)
            d.contentMetadata={
                                    "STKID": "22050058",
                                    "STKPKGID": "1673952",
        ...
        if op.type == 26:
                 ...
                 if wait["detectMention"] == True:
                     ...
                     for mention in mentionees:
                           if mention['M'] in Bots:
                                  ...
                                  iyo.sendMessage(msg)
                                  iyo.sendText(msg.to,"mamvos... gua kash virus... 凸(-_-)凸.")               
                                  break   
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = iyo.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["idih si " + cName + "ngetag gua"]
                    balas1 = "ada apa " + cName + "jones...\n\n\nneh potonya si jones yg suka ngetag..."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
        ...
        if op.type == 26 or op.type == 25:
            ...
            if msg.text in ["Bot2 on"]:
              if msg.from_ in Creator:    
                wait["Bot2"] = True
                iyo.sendText(msg.to,"Bot2 Sudah On Kembali.")               
        ...
        if op.type == 26:
          if wait["Bot2"] == True:    
            ...
            if msg.contentType == 7:       
                iyo.sendText(msg.to,"tikel 10c aja pamer... 🤣")                
   

        if op.type == 26:
            msg = op.message            
            if msg.text in ["Mayhem","mayhem","Nuke","nuke","Greet","greet","Ratakan","ratakan","Ready op","ready op","Cleanse","cleanse","Kickall","kickall",".kickall","!kickall","Ready op"]:
              dia = ("Tercyduck Kamu Yah Mau nakal 😆","jangan sok jd kicker njer.. ╭∩╮")
              ket = random.choice(dia)
              iyo.sendText(msg.to,ket)
              iyo.kickoutFromGroup(msg.to,[msg.from_])
              iyo.sendText(msg.to,"Mampus Lu Kekick 😎")     
...
#KICK_BY_TAG    
		...
		if msg.from_ in admin:
		    ...
		    iyo.sendText(msg.to,"Khusus admin")
            ...
            elif msg.text in ["Sambitan on","Wm on"]:
                if wait["Sambutan"] == True:
                    if wait["lang"] == "JP":
                        iyo.sendText(msg.to,"Nyambit Di Aktifkanヾ(*´∀｀*)ﾉ")
                else:
                    wait["Sambutan"] = True
                    if wait["lang"] == "JP":
                        iyo.sendText(msg.to,"Sudah Onヽ(´▽｀)/")

            elif msg.text in ["Sambitan off","Wm off"]:
                if wait["Sambutan"] == False:
                    if wait["lang"] == "JP":
                        iyo.sendText(msg.to,"Nyambit Di Nonaktifkan(　＾∇＾)")
                else:
                    wait["Sambutan"] = False
                    if wait["lang"] == "JP":
                ...
                wait["Sider"] = True
                iyo.sendText(msg.to,"Tikungan sudah on")
                
            elif "Tikungan off" in msg.text:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    iyo.sendText(msg.to, "ko di matiin bos? cape y nikung mulu...😅")
                else:
                ...
                md = ""
		if wait["Sambutan"] == True: md+="╠❂͜͡➣✔️ Nyambit : On\n"
		else:md+="╠❂͜͡➣❌ Nyambit : Off\n"
		...
		if wait["Sider"] == True: md+="╠❂͜͡➣✔️ Nikung : On\n"
		else:md+="╠❂͜͡➣❌ Nikung : Off\n"	
		if wait["Simi"] == True: md+="╠❂͜͡➣✔️ Simisimi : On\n"
		else:md+="╠❂͜͡➣❌ Simisimi: Off\n"		
		if wait["Bot"] == True: md+="╠❂͜͡➣✔️ Bot : On\n"
		else:md+="╠❂͜͡➣❌ Bot: Off\n"		
		if wait["Bot2"] == True: md+="╠❂͜͡➣✔️ Bot2 : On\n"
		else:md+="╠❂͜͡➣❌ Bot2: Off\n"		
		...
		if msg.from_ in admin:	 	        
            ...
            elif msg.text in ["Of dulu"]:
                wait["copy"] = True
                iyo.sendText(msg.to,"Send Contact")
                msg.contentType = 13
                msg.contentMetadata = {'mid': "uf84cedaf7bd7f637a1d029612a4d36fd"}
                iyo.sendMessage(msg) 
		...
		iyo.sendText(msg.to, "Progress...\n█████████▒90%")
                iyo.sendText(msg.to, "%s" % (elapsed_time))
                
            elif msg.text in ["Speed test","Cek"]:
                start = time.time()
                iyo.sendText(msg.to, "f̸l̸a̸s̸h̸...")
                elapsed_time = time.time() - start
                iyo.sendText(msg.to, "%s" % (elapsed_time))                
		...
		if msg.from_ in Creator:
            ...
            elif "Off dulu" in msg.text:
                if msg.from_ in admin:
                    path = "iyo.jpg"
                    path2 = "off untuk sementara"
                    iyo.sendText(msg.to,"Proses...")
                    #iyo.sendImage(msg.to,path)
                    iyo.updateProfilePicture(path)
                    #iyo.updateProfileStatus(path2)
                    iyo.CloneContactProfile(target)
                    iyo.sendText(msg.to,"Done. . .")
                else:
                    iyo.sendText(msg.to, "Khusus Admin")
	    ...
	    elif "Bot2 off" in msg.text:
	     if msg.from_ in Creator:    
	        wait["Bot2"] = False
	    	iyo.sendText(msg.to,"Bot2 sudah off")
	     else:
		    iyo.sendText(msg.to,"Khusus iyo")	    	


	    elif "Allbot off" in msg.text:
	     if msg.from_ in Creator:    
	        wait["Bot"] = False
	        wait["Bot2"] = False
	    	iyo.sendText(msg.to,"Semua Bot sudah off")
	     else:
		    iyo.sendText(msg.to,"Khusus iyo")	    	
