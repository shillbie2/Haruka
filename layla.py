...
iyo = LINETCR.LINE()
#iyo.login(qr=True)
iyo.login(token='Eqk6xTLM9VvfXiHdtli6.jA4WUqEDJNBkq3rQlCK+fG.RlkRDzZjlQlrvP0sBOuetEiTIO8iQw694R/31kGm0xY=')
iyo.loginResult()
    ...
    try:
                        ...
                        if op.param1 in cctv['point']:
                            ...
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        iyo.sendText(op.param1, "eh si.. " + "☞ " + nick[0] + " ☜" + "\nbaru dateng. . .\nKemana ja kak?   ")
                                    else:
                                        iyo.sendText(op.param1, "nah kan.. si " + "☞ " + nick[1] + " ☜" + "\nn0ng0L juga. . .\nkyanya bwa jajanan nh.. (*´∇｀*)   ")
                                else:
                                    iyo.sendText(op.param1, "jiah.. ngapain kak " + "☞ " + Name + " ☜" + "\nNgintip aja???\nSini Gabung Chat... biar kita kompak.. ＼（○＾ω＾○）／   ")
                        else:
                            pass
                    else:
                        pass
        ...
        if op.type == 17:
          if wait["Sambutan"] == True:
            ...
            iyo.sendText(op.param1,"Hay " + iyo.getContact(op.param2).displayName + "\nSalam kenal N met gabung di room  ☞ " + str(ginfo.name) + " ☜" + "Smoga betah dan jgn lupa \nbudayakan cek note yaa kk... ")
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in Creator:
                return
            iyo.sendText(op.param1,"selamat jalan.. " + iyo.getContact(op.param2).displayName +  "\nsayang ,Smoga bertemu di lain kesdmpatan...")
            d = Message(to=op.param1, from_=None, text=None, contentType=7)
            d.contentMetadata={
                                    "STKID": "11389596",
                                    "STKPKGID": "6505",
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = iyo.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["eh si.. " + cName + ", Ngetag Mulu!"]
                    balas1 = "Ini Foto tukang tag . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
		...
		if msg.from_ in Creator:
            ...
            elif msg.text in ["Sijgmgwmisimi on","Simijgjgjsimi:on"]:
