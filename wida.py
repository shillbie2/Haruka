...
iyo = LINETCR.LINE()
#iyo.login(qr=True)
iyo.login(token='EqR3OgVtu9PwOViN3QNa./qAiHtLLK6xwkUIGVrDmAG.xXgSdJNqXN2DYnFt3BF+zBfO2F6orH7OipTkbpwb1Zs=')
iyo.loginResult()
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = iyo.getContact(msg.from_)
                    cName = contact.displayName
                    balas = [cName + "\nnanti ketularan virus typo loh....."]
                    ret_ = "jangan tag gua..\n" + random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = iyo.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["ada apa kak.. " + cName + ", panggil²?\n lg nanggung nh.."]
